...
import AddSensore from './AddSensore'
...
export default class Sensore extends Component {
    state = {
        acessoemps: [],
        usuario: [],
        visible: [],
...
//---------------------------------------------------------------------------------------
    AddSensores = async  locData  => {
        this.setState({loading: true})
        try {
            const res = await axios.post(`${server}/sensores`, {
                sensoresDataCadastro: new Date(), 
...
//--------------------------------------------------------------------------------------- 
    updateSensores = async  sensore  => {
        try {
            await axios.put(`${server}/sensores/atualisa`, {
                sensores_id: dados.id,
                sensoresLocal:sensore.sensoresLocal,
                sensoresEquipamento: sensore.sensoresEquipamento,
                sensoresLimpeza: sensore.sensoresLimpeza,
                sensoresSituacao: sensore.sensoresSituacao,
                sensoresResponsavel: sensore.sensoresResponsavel,
                sensoresPrevencao: sensore.sensoresPrevencao,
                sensoresObservacao: sensore.sensoresObservacao,
                sensoresDataUpdate: new Date()
            })
        ...
        if(this.state.loading){
            ...
            return (
                <View style={styles.container}>
                    <AddSensore {...this.state} isVisible={this.state.showAdd} 
                        onSave={this.updateSensores}
                    ...
                    <View style={styles.taksContainer}>
                        <Text style={styles.titletela}>Sensores</Text>
                        <View style={styles.linha} />
                    </View>
