...
export default class Cofre extends Component {
    state = {
        acessoemps: [],
        usuario: [],
        visible: [],
...
//---------------------------------------------------------------------------------------
    AddCofres = async  locData  => {
        this.setState({loading: true})
        try {
            const res = await axios.post(`${server}/cofres`, {
                cofresDataCadastro: new Date(), 
...
//--------------------------------------------------------------------------------------- 
    updateCofres = async  cofre  => {
        try {
            await axios.put(`${server}/cofres/atualisa`, {
                cofres_id:            dados.id,
                cofresValor:          cofre.cofresValor,
                cofresDiferenca:      cofre.cofresDiferenca,
                cofresDocResponsavel: cofre.cofresDocResponsavel,
                cofresDocAuditor:     cofre.cofresDocAuditor,
                cofresObservacao:     cofre.cofresObservacao,
                cofresDataUpdate:     new Date()
            })
        ...
        if(this.state.loading){
            ...
            return (
                <View style={styles.container}>
                    <AddCofre {...this.state} isVisible={this.state.showAdd} 
                        onSave={this.updateCofres}
                    ...
                    <View style={styles.taksContainer}>
                        <Text style={styles.titletela}>Cofres</Text>
                        <View style={styles.linha} />
                    </View>
