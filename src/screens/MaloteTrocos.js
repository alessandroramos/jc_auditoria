...
import AddMaloteTroco from './AddMaloteTroco'
...
export default class MaloteTroco extends Component {
    state = {
        acessoemps: [],
        usuario: [],
        visible: [],
...
//---------------------------------------------------------------------------------------
    AddMaloteTrocos = async  locData  => {
        this.setState({loading: true})
        try {
            const res = await axios.post(`${server}/malote_trocos`, {
                malote_trocosDataCadastro: new Date(), 
...
//--------------------------------------------------------------------------------------- 
    updateMaloteTrocos = async  malote_trocos  => {
        try {
            await axios.put(`${server}/malote_trocos/atualisa`, {
                malote_trocos_id: dados.id,
                malote_trocosOperador:    malote_trocos.malote_trocosOperador,
                malote_trocosValor:       malote_trocos.malote_trocosValor,
                malote_trocosDocOperador: malote_trocos.malote_trocosDocOperador,
                malote_trocosResponsavel: malote_trocos.malote_trocosResponsavel,
                malote_trocosObservacao:  malote_trocos.malote_trocosObservacao,
                malote_trocosDataUpdate:  new Date(),
                malote_trocosAuditor:     malote_trocos.malote_trocosAuditor
            })
        ...
        if(this.state.loading){
            ...
            return (
                <View style={styles.container}>
                    <AddMaloteTroco {...this.state} isVisible={this.state.showAdd} 
                        onSave={this.updateMaloteTrocos}
                    ...
                    <View style={styles.taksContainer}>
                        <Text style={styles.titletela}>MaloteTrocos</Text>
                        <View style={styles.linha} />
                    </View>
