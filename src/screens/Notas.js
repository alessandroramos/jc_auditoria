...
//---------------------------------------------------------------------------------------
    AddNotas = async  locData  => {
        this.setState({loading: true})
        try {
            const res = await axios.post(`${server}/notas`, {
                notasDataCadastro: new Date(), 
...
//--------------------------------------------------------------------------------------- 
    updateNotas = async  nota  => {
        try {
            await axios.put(`${server}/notas/atualisa`, {
                notas_id: dados.id,

                notasNumero:      nota.notasNumero,
                notasObs:         nota.notasObs,
                notasQuantidade:  nota.notasQuantidade,
                notasPrevencao:   nota.notasPrevencao,
                notasObservacao:  nota.notasObservacao,
                notasResponsavel: nota.notasResponsavel,
                notasDataUpdate: new Date()
            })
        ...
        if(this.state.loading){
            ...
            return (
                <View style={styles.container}>
                    <AddNota {...this.state} isVisible={this.state.showAdd} 
                        onSave={this.updateNotas}
                    ...
                    <View style={styles.taksContainer}>
                        <Text style={styles.titletela}>Notas</Text>
                        <View style={styles.linha} />
                    </View>
